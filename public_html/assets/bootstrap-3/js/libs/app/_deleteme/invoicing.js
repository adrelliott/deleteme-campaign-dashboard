...
* */
 ...
 var updatePrice = function($itemRow){
    ...
    var price = $itemRow.find('#itemPrice').val().replace("Â£", "") * $itemRow.find('#itemQty').val();
...
var update_total = function() {
    ...
    $('input#itemLineTotal').each(function (i) {
        price = $(this).val().replace("Â£", "");
        if (!isNaN(price)) total += Number(price);
    });

    total = roundNumber(total, 2);
    //$('#invGrandTotalTop, #invGrandTotal').html("<h4>Â£" + total + "</h4>");
...
var update_grand_total = function() {
    ...
    running_total = 0;
     $('input.subtotal').each(function (i) {
        running_total = $(this).val().replace("Â£", "");
